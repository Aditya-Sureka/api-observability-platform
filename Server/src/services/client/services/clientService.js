 ...
 * This class is responsible for creating clients, managing client users, and handling API Keys for clients.
 */
export class ClientService {
  ...
  async createClientUser(clientId, userData, adminUser) {
    try {
      ...
      if (!isValidClientRole(role)) {
        throw new AppError(
          `Invalid role: ${role}. Must be one of ${Object.values(APPLICATION_ROLES).join(", ")}`,
          400,
        );
      }
   ...
   */
  async createClientApiKey(clientId, keyData, user) {
    try {
      const client = await this.clientRepository.findById(clientId);
