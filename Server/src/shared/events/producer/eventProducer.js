...
export class EventProducer {
  ...
  async publishApiHit(eventData, opts = {}) {
    ...
    if (!this._circuitBreaker.allowRequest()) {
      this._logger.info(
        "[EventProducer] Circuit breaker is open, rejecting publish request",
        {
          eventId: eventData.eventId,
    ...
    while (true) {
      try {
        ...
        this._logger.info("[EventProducer] Event published successfully", {
          eventId: eventData.eventId,
          correlationId,
          attempt: attempt + 1,
          latencyMs,
        ...
        this._logger.error("[EventProducer] Failed to publish event", {
          eventId: eventData.eventId,
          correlationId,
          attempt: attempt + 1,
          error: error.message,
