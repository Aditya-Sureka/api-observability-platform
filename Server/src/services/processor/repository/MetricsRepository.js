...
const MAX_LIMIT = 1000; // Define a maximum limit for the number of records to retrieve
...
export class MetricsRepository extends BaseRepository {
  ...
  async upsertEndpointMetrics(metricsData) {
    try {
      ...
      this.logger.error(
        "Error occurred while upserting endpoint metrics:",
        error,
      );
      throw error;
    }
  ...
  async getMetrics(filter = {}) {
    try {
      ...
      const safeLimit = Math.min(Math.max(1, limit), MAX_LIMIT); // Ensure limit does not exceed MAX_LIMIT
      const safeOffset = Math.max(0, offset); // Ensure offset is non-negative
  ...
  async getOverallStats(clientId, startTime = null, endTime = null) {
    try {
      ...
      this.logger.error("Error occurred while fetching overall stats:", error);
      throw error;
    }
  }
