...
class EventConsumer {
  ...
  async start() {
    try {
      ...
      this.channel.on("close", () => {
        this._logger.warn("Consumer channel closed");
      ...
      this._logger.info("Consumer is now listening for messages...");
    } catch (error) {
      this._logger.error("Error starting consumer:", error);
      await this._cleanup();
      throw error;
    }
  }
  ...
  async _cleanup() {
    try {
      ...
      this._logger.error("Error during consumer cleanup:", error);
    }
  }
  ...
  async _connectDatabases() {
    ...
    while (retries < maxRetries) {
      try {
        ...
        this._logger.info("Connected to MongoDB and PostgreSQL successfully.");
        return;
      } catch (error) {
        retries++;
        this._logger.error(
        ...
        if (retries >= maxRetries) {
          throw new Error(
            `Failed to connect to MongoDB and PostgreSQL after ${maxRetries} attempts.`,
          );
        }
        await new Promise((resolve) => setTimeout(resolve, 5000 * retries)); // Wait before retrying
      }
    }
  }
  ...
  async _reconnect() {
    try {
      await new Promise((resolve) => setTimeout(resolve, 5000)); // Wait before reconnecting
      ...
      this._logger.error("Error reconnecting to RabbitMQ:", error);
      if (this.isRunning) {
        setTimeout(() => this._reconnect(), 10000); // Retry after 10 seconds
      }
    }
  }
  ...
  async _handleMessage(msg) {
    ...
    try {
      ...
      if (this._processedIds.has(messageData.messageId)) {
        this._logger.info(
          `Duplicate message detected. Acknowledging messageId: ${messageData.messageId}`,
        );
  ...
  async _handleProcessingError(error, msg, messageData, startTime) {
    ...
    if (poisonCount >= 10) {
      this._logger.error("Poison message detected for event type:", {
        eventType,
        consecutiveFailures: poisonCount,
      });
    }

    // Non-retryable error go straight to DLQ
    ...
    await this._retryStrategy(msg, retryCount);
  }
  ...
  async _sendToDLQ(msg, error, reason) {
    try {
      ...
      this.channel.nack(msg, false, true); // Requeue the message for another attempt
    }
  }

  async _retryMessage(msg, retryCount) {
    const delay = this._retryMessage.delay(retryCount);
    ...
    setTimeout(() => {
      try {
        ...
        this._logger.error("Failed to retry message:", error);
  ...
  while (startupRetry.shouldRetry(attempt) || attempt === 0) {
    try {
      ...
      if (!startupRetry.shouldRetry(attempt)) {
        logger.error("Max startup attempts reached. Exiting process.");
        process.exit(1);
      }
