...
export class ProcessorService {
  ...
  async processEvent(eventData) {
    ...
    try {
      ...
      // yeh success hoga ya fir pura operation fail hoga
      await this.apiHitRepository.save(eventData);
      rawEventSaved = true;

      logger.info("Event data saved successfully to MongoDB", {
        eventId: eventData.eventId,
      });
      ...
      if (!rawEventSaved) {
        logger.error("Error occurred while saving event data to MongoDB", {
          error: error.message,
          eventId: eventData.eventId,
        });
        throw error;
