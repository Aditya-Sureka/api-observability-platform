...
export class CircuitBreaker {
  ...
  allowRequest() {
    ...
    if (current === CircuitState.HALF_OPEN) {
      ...
      this.logger.warn(
        `[CircuitBreaker] HALF_OPEN max attempts reached (${this._halfOpenAttempts}/${this.halfOpenMaxAttempts})`,
      );
      return false;
    }
  ...
  onSuccess() {
    ...
    if (this._failures > 0) {
      this._failures = 0;
      this.logger.info("[CircuitBreaker] reset failures to 0 after success");
    }
  }
  ...
  onFailure() {
    ...
    if (this._state === CircuitState.HALF_OPEN) {
      this.logger.warn(
        "[CircuitBreaker] HALF_OPEN failure recorded, reopening circuit",
      );
      this._openCircuit();
      return;
    }
