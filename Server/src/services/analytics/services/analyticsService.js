...
export class AnalyticsService {
  constructor(metricsRepo) {
    if (!metricsRepo)
      throw new Error(
        "AnalyticsService requires a metrics repository instance",
      );
  ...
  async getOverallStats(clientId, filters = {}) {
    try {
      ...
      logger.error("Error occurred while fetching overall stats:", error);
      throw error;
    }
  }
  ...
  async getTopEndpoints(clientId, options = {}) {
    try {
      ...
      logger.error("Error occurred while fetching top endpoints:", error);
      throw error;
    }
  }
  ...
  async getTimeSeries(clientId, filters = {}) {
    try {
      ...
      const metrics = await this.metricsRepository.getTimeSeries({
        clientId,
        serviceName,
        endpoint,
      ...
      logger.error("Error occurred while fetching time series metrics:", error);
      throw error;
    }
  }
}
