...
export class AnalyticsController {
  constructor({
    ...
    if (!analyticsSvc || !authSvc || !clientRepo)
      throw new Error("AnalyticsController requires all required dependencies");
      ...
      res
        ...
        .json(
          ResponseFormatter.success(
            stats,
            "Overall stats fetched successfully",
            200,
          ),
        );
    } catch (error) {
  ...
  validateTimeRange(startTime, endTime) {
    ...
    if ((startTime && Number.isNaN(start)) || (endTime && Number.isNaN(end))) {
      throw new AppError("Invalid time range provided", 400);
    }
  ...
  async ensureCanViewAnalytics(req) {
    ...
    if (
      ...
      throw new AppError(
        "Forbidden: Insufficient permissions to view analytics",
        403,
      );
    }
    return false;
  ...
  async resolveFinalClientId(req, isSuperAdmin) {
    ...
    if (isSuperAdmin) {
      if (queryClientId) {
        if (!this.isValidObjectId(queryClientId)) {
          throw new AppError("Invalid clientId provided", 400);
        }
    ...
    if (!userClientId) {
      throw new AppError("User does not have an associated clientId", 403);
    }

    if (!this.isValidObjectId(userClientId)) {
      throw new AppError("Invalid user clientId", 400);
    }
